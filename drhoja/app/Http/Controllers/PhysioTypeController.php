...
class PhysioTypeController extends Controller
{
    public function index(){
        return view('type');
    }

    public function store(PhysioTypeRequest $request)
    {

        $res = PhysioTypes::updateOrCreate(
            ['id' => $request->row_id],
            ['title' => $request->add_type_name,
                'status' => $request->add_type_status]);
        //Log::debug($res);
    ...
    {
        try {
            $types = PhysioTypes::findOrfail($id);
    ...
    {
        try {
            PhysioTypes::findOrfail($id)->delete();
        } catch (\Illuminate\Database\QueryException $e) {
            if ($e->errorInfo[1] == 1451) {
                return response()->json(['code' => 406], 406);
            }
