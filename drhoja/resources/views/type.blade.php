...
@section('content')
    ...
    <div class="col-span-12 md:col-span-7 lg:col-span-8 xl:col-span-9 flex flex-col gap-2 overflow-auto mt-5">
        ...
        <div class="flex flex-col sm:flex-row justify-start gap-2 mt-3 w-full">
            <a onclick="showModal('add_type')" class="min-w-[100px] text-center bg-colorprimary text-white sm:hover:-translate-y-1 transition-transform duration-200 py-3 px-5 rounded-full cursor-pointer text-xs sm:text-sm">افزودن درمان</a>
        </div>
        <!-- --------------------------table -------------------  -->
        <div class="mt-2">
            ...
            <div class="gap-2 rounded-xl">
                ...
                <table id="type_datatable"  class="space-y-3 block">
                    ...
                    <tr class="flex justify-between items-center w-full bg-white font-extrabold px-1 md:px-5 py-3 rounded-lg hover:shadow-gray-200/50 hover:shadow-lg text-xs lg:text-sm">
                        ...
                        <td class="w-[60%] md:w-[25%] lg:w-[40%] text-center border-0 sm:border-l">عنوان درمان</td>
    ...
    <!-- ---------- مودال تعریف عنوان  ------------------  -->
    <div id="add_type" x-transition.duration.500ms class="bg-black/50 hidden backdrop-blur-sm fixed pb-10 top-0 w-full h-screen flex items-center px-5 z-50">
        <div class="bg-white w-full sm:max-w-2xl max-h-[90vh] sm:max-h-full mx-auto p-5 rounded-lg flex flex-col gap-3 overflow-y-auto">
                <span class="flex flex-row justify-between items-center border-b pb-2">
                     <span>تعریف نوع درمان</span>
                     <span onclick="closeModal('add_type')" class="bg-gray-100 p-1 sm:p-2 rounded-md cursor-pointer">
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-2 mt-3 w-full">
                ...
                <a onclick="closeModal('add_type')" class="min-w-[100px] bg-gray-600 text-white sm:hover:-translate-y-1 transition-transform duration-200 py-3 px-5 rounded-full cursor-pointer text-xs sm:text-sm text-center">بازگشت</a>
            </div>
        </div>
    </div>
...
@section('script')
    ...
    <script>
        ...
        function filterList() {
            laravel_datatable = $('#type_datatable').DataTable({
                processing: true,
                serverSide: true,
                destroy: true,
                responsive: true,
        ...
        function editRow(id) {
            let _url            = 'type/' + id;
            let status_elmnt    = document.getElementById("add_type_status");
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response) {
                        console.log('response');
                        $('#add_type_name').val(response.title);
